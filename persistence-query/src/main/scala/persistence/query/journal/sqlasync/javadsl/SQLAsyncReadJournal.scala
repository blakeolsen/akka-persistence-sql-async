package persistence.query.journal.sqlasync.javadsl
...
/**
 * Java API: [[akka.persistence.query.javadsl.ReadJournal]] implementation for SQLAsync.
 *
 * It is retrieved with:
 * {{{
 * SQLAsyncReadJournal queries =
 *   PersistenceQuery.get(system).getReadJournalFor(SQLAsyncReadJournal.class, SQLAsyncReadJournal.Identifier());
 * }}}
 *
 * Corresponding Scala API is in [[persistence.query.journal.sqlasync.scaladsl.SQLAsyncReadJournal]].
 *
 * Configuration settings can be defined in the configuration section with the
 * absolute path corresponding to the identifier, which is `"akka-persistence-sql-async.query"`
 * for the default [[SQLAsyncReadJournal#Identifier]]. See `reference.conf`.
 *
 */
class SQLAsyncReadJournal(scaladslReadJournal: persistence.query.journal.sqlasync.scaladsl.SQLAsyncReadJournal)
  extends ReadJournal
   ...
   *
   * The SQLAsync write journal is notifying the query side as soon as new `persistenceIds` are
   ...
   *
   * The SQLAsync write journal is notifying the query side as soon as events are persisted, but for
   ...
   *
   * The SQLAsync write journal is notifying the query side as soon as tagged events are persisted, but for
   * efficiency reasons the query side retrieves the events in batches that sometimes can
   * be delayed up to the configured `refresh-interval`.
   *
   * The stream is completed with failure if there is a failure in executing the query in the
...
object SQLAsyncReadJournal {
  /**
   * The default identifier for [[SQLAsyncReadJournal]] to be used with
   * [[akka.persistence.query.PersistenceQuery#getReadJournalFor]].
   *
   * The value is `"persistence.query.journal.sqlasync"` and corresponds
   * to the absolute path to the read journal configuration entry.
   */
  final val Identifier = persistence.query.journal.sqlasync.scaladsl.SQLAsyncReadJournal.Identifier
}
