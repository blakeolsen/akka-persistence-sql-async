package persistence.query.journal.sqlasync.scaladsl
...
/**
 * Scala API [[akka.persistence.query.scaladsl.ReadJournal]] implementation for SQLAsync.
 *
 * It is retrieved with:
 * {{{
 * val queries = PersistenceQuery(system).readJournalFor[SQLAsyncReadJournal](SQLAsyncReadJournal.Identifier)
 * }}}
 *
 * Corresponding Java API is in [[persistence.query.journal.sqlasync.javadsl.SQLAsyncReadJournal]].
 *
 ...
 */
class SQLAsyncReadJournal(system: ExtendedActorSystem, config: Config) extends ReadJournal
   ...
   */
    override def allPersistenceIds(): Source[String, Unit] = {
      // no polling. The write journal will push updates
...
object SQLAsyncReadJournal {
   ...
   */
  final val Identifier = "persistence.query.journal.sqlasync"
}
